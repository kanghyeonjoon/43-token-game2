...
이 서버는 개인 PC에서만 쓰는 로컬 전용 프로그램이며, 아래 보호 장치가 적용되어 있습니다.

- **로컬 전용 바인딩**: 서버는 `127.0.0.1`에만 열리므로 같은 네트워크의 다른 기기에서 접속할 수 없습니다.
- **CSRF 토큰 검증**: 계정 목록을 바꾸는 모든 POST 요청은 서버가 페이지에 심어준 1회성 토큰(`X-Csrf-Token` 헤더)이 있어야만 처리됩니다. 서버를 켠 채로 악의적인 웹사이트를 방문해도 그 사이트가 이 서버의 설정을 조작할 수 없습니다.
- **Host 헤더 검증**: `localhost`/`127.0.0.1` 이외의 Host로 들어온 요청은 모두 거부해 DNS 리바인딩 공격을 차단합니다.
- **이미지 프록시 화이트리스트**: `/api/img`는 인스타·유튜브·틱톡 등 공식 CDN의 https 주소만 대신 가져옵니다.
- 결제·카드·로그인 정보를 다루는 코드는 없으며, 프로그램이 쓰는 파일은 자기 폴더 안의 구독 계정 목록 JSON뿐입니다.
